    ...
    return (
        <footer id='contact' className='white_spacing'>
            <div className={`${styles.top_footer} _flex`}>
                        ...
                        return <FooterLinks key={index} title={linkInfo.title} >
                            {
                                linkInfo.links?.map((link,index)=>{
                                    return <li key={index}><HashLink smooth to={link.url} className={`${styles.link} _text_decoration footer_shift`}>{link.content}</HashLink></li>
                                })
                            }
                            </FooterLinks>
                    })
                ...
                <div>
                    <FooterTitle title={location.title}/>
                    <a target='_blank' className={`${styles.location} footer_shift`} href='https://www.google.com/maps?q=Damascus'
                    >2972 Westheimer Rd. Santa Ana, Illinois 85486 </a>
