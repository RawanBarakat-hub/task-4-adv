                    ...
                    <ul className={`${styles.links} _flex _list_style_none`}>
                        {
                            items?.map((item,index)=>{
                                return <li key={index}><HashLink sm to={item.url} className={({ isActive }) =>`${styles.nav_link} ${isActive && styles.active} shift_header _text_decoration`}>{item.content}</HashLink></li>
                            })
                        }
                    </ul>
